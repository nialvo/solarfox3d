...
//   shipGeometry.rotateX(Math.PI / 2); // Rotate the geometry so it faces -Z
  
//   const shipMaterial = new THREE.MeshStandardMaterial({ color: 0x990033 });
//   const ship = new THREE.Mesh(shipGeometry, shipMaterial);
//   ship.position.set(0, 10, 0); // No rotation needed on the mesh now

//   // 🎥 Camera as child of ship
//   camera.position.set(0, -12, 2); // behind 
//   camera.rotation.x = Math.PI/2.5;
//   ship.add(camera);

//   scene.add(ship); // Add ship (with camera inside) to scene
...
//     // Rotation
//     if (keys['Numpad4']) ship.rotation.y += rotSpeed; // Yaw left
//     if (keys['Numpad6']) ship.rotation.y -= rotSpeed; // Yaw right
//     if (keys['Numpad8']) ship.rotation.x += rotSpeed; // Pitch up
//     if (keys['Numpad2']) ship.rotation.x -= rotSpeed; // Pitch down
//     if (keys['Numpad7']) ship.rotation.z += rotSpeed; // Roll left
//     if (keys['Numpad9']) ship.rotation.z -= rotSpeed; // Roll right
...
window.onload = () => {
  ...
  // Ship
  const shipGeometry = new THREE.ConeGeometry(0.5, 1, 8);
  shipGeometry.rotateX(Math.PI / 2); // face -Z
  const shipMaterial = new THREE.MeshStandardMaterial({ color: 0x990033 });
  const ship = new THREE.Mesh(shipGeometry, shipMaterial);
